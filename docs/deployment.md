...
Версия схемы хранится в `PRAGMA user_version`, недостающие миграции применяются при старте, перед миграцией бот делает бэкап файла базы. Перед обновлением на проде всё равно сделайте свою копию. Проще всего скриптом, он снимает копию онлайн и сам подчищает старые:

```bash
./scripts/backup.sh
```

Для контейнера подойдёт и обычное копирование файла:
...
Очистку кеша и обновление из GitHub тоже можно не делать руками: смотрите [scripts.md](scripts.md).
