...
# Название семьи/сообщества. Подставляется во все тексты, которые видит
# участник: панель заявок, личные сообщения, имя категории тикетов.
# Задаётся через .env, чтобы один и тот же код работал у разных семей.
FAMILY_NAME = _raw_env("FAMILY_NAME") or "FAMQCORE"
...
TICKETS_CATEGORY_NAME = f"{FAMILY_NAME} • заявки"
...
# Каналы
# Имя канала для legacy-режима «один канал с ветками» (когда задан
# LOG_CHANNEL_ID, бот в него только пишет; имя нужно лишь для сообщений).
...
# Лог-центр: приватная категория, внутри отдельный канал на каждую функцию
# (в legacy-режиме с LOG_CHANNEL_ID это один канал с ветками).
...
# Имена разделов лога. В управляемом режиме это имена каналов внутри
# категории LOG_CATEGORY_NAME, в legacy-режиме это имена веток в одном канале.
LOG_SECTION_NAMES = {
...
# Совместимость: старое имя словаря.
LOG_THREAD_NAMES = LOG_SECTION_NAMES

# Категория, в которой бот создаёт приватные каналы логов (управляемый режим).
LOG_CATEGORY_NAME = "logs bot"

# При записи в раздел ошибок бот пингует ответственных: роли владельцев
# из .env и фактического владельца сервера. Отключается флагом.
LOG_ERROR_PING_ENABLED = True
LOG_ERROR_PING_GUILD_OWNER = True

# Короткая памятка о баг-репортах, которую бот публикует при создании
# канала ошибок: чтобы дежурный знал, что прикладывать к задаче.
LOG_ERRORS_GUIDE = (
    "**Как сообщать об ошибках бота**\n"
    "1. Скопируйте сообщение об ошибке целиком, включая код (`correlation_id`).\n"
    "2. Найдите этот код в файле `logs/bot.log` и возьмите строку с ним "
    "и пару строк вокруг.\n"
    "3. Опишите, что делали: команда или кнопка, канал, что ожидали увидеть.\n"
    "4. Создайте задачу в трекере проекта и приложите пункты 1-3.\n"
    "5. Не выкладывайте в задачу токен бота, ID приватных каналов "
    "и персональные данные участников."
)
...
# Тексты
DM_MESSAGE = f"Вы подали заявку в {FAMILY_NAME}, ожидайте, скоро её рассмотрят ⏳."

FAMQCORE_EMBED_TITLE = FAMILY_NAME
PRIVACY_NOTICE = (
...
FAMQCORE_EMBED_DESCRIPTION = (
    f"Добро пожаловать в {FAMILY_NAME}.\n\n"
...
# Discord запрещает менять ник владельцу сервера и тем, чья роль выше роли
# бота. AFK в этом случае всё равно включается, но участника нужно об этом
# предупредить, иначе он думает, что команда не сработала.
AFK_NICK_NOT_APPLIED_NOTE = (
    "ℹ️ Префикс [AFK] к нику не добавлен: у бота нет прав менять ваш ник "
    "(так бывает у владельца сервера и у ролей выше роли бота). "
    "Сам AFK-статус работает."
)
